...
export interface SectionCardProps {
    ...
    figText?: string;     // Custom figure or mission text
}
interface FallState {
...
const useBreakableCard = (onBreak?: () => void) => {
    ...
    const handleDragMove = useCallback((clientX: number, clientY: number) => {
        ...
        if (isHorizontalImpact) {
            ...
            setIsFlashing(true);
            setTimeout(() => setIsFlashing(true), 0); // Trigger flash
 ...
 * =========================================================================
 */
export const BreakableCard = ({ title, description, className, onBreak, figText }: SectionCardProps) => {
    const {
        isDragging,
        position,
        crackLevel,
    ...
    return (
        <div className={cn("relative w-full h-full min-h-[300px]", className)}>
            ...
            */}
            <div className="absolute inset-0 bg-[#EAE3CB] border-2 border-dashed border-[#88734C]/30 rounded-3xl flex flex-col items-center justify-center z-0">
                <span className="text-[#88734C]/60 font-black text-2xl uppercase tracking-widest mb-2">Gone</span>
                {/* Loading Bar (Visible only when waiting for respawn) */}
                {isBroken && (
                    <div className="w-24 h-4 border border-[#88734C]/20 bg-[#FAF7F0] rounded relative overflow-hidden mt-2">
                        <div
                            className="absolute inset-0 bg-[#88734C]/40 transition-all duration-75 ease-linear"
            ...
            */}
            <div
                ...
                className={cn(
                    "relative z-10 bg-white/40 backdrop-blur-xl border border-dark-green/10 rounded-[2.5rem] p-8 shadow-xl hover:shadow-2xl hover:bg-white/50 cursor-grab active:cursor-grabbing select-none h-full flex flex-col justify-between overflow-hidden transition-all duration-300",
            ...
            >
                <div>
                    ...
                    <div className="flex justify-between items-start mb-6">
                        <h3 className="font-black text-[30px] uppercase tracking-tighter leading-none text-orange">{title}</h3>
                    </div>
                    {/* Body */}
                    <div className="font-semibold text-[20px] leading-relaxed text-[#202e44]/90 whitespace-pre-wrap">
                            ...
                            description.split('\n').map((line, idx) => {
                                ...
                                return (
                                    <div 
                                      key={idx} 
                                      className={cn(
                                        isStar ? "text-[17px] font-medium text-[#202e44]/75 mt-1" : "text-[20px]"
                                      )}
                                    >
                                      {line}
                                    </div>
                ...
                {/* Footer */}
                <div className="mt-8 border-t border-black/10 pt-2 flex justify-between text-xs font-mono uppercase">
                    <span>{figText || `Fig. ${figNum.toString().padStart(2, '0')}`}</span>
